...
func testStringObject(expected string, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("object has the wrong value. got = %q want=%q ", result.Value, expected)
	}
	return nil
}
...
func testCallingFunctionsWithBindings(t *testing.T) {
	tests := []vmTestCase{
		...
		{
			input: `let globalSeed = 50; let minusOne = fn() {
				let num = 1;
					globalSeed - num;
				}
				let minusTwo = fn() {
...
func TestCallingFunctionsWithWrongArguments(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if err.Error() != tt.expected {
			t.Fatalf("wrong VM error: expected %s, actual %s", tt.expected, err)
		}
	}
	// runVmTests(t, tests)
}
