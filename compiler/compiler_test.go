...
func runCompilerTests(t *testing.T, tests []compilerTestCase) {
	for _, tt := range tests {
		// fmt.Printf("running test %q", tt.input)
...
func TestFunctionsWithReturnValues(t *testing.T) {
	tests := []compilerTestCase{
		{
			input: `fn() { }`,
			expectedConstants: []interface{}{
...
// 		t.Errorf("instrcutions length wrong. got=%d", len(compiler.scopes[compiler.scopeIndex].instructions))
// 	}
...
func TestLetStatementScopes(t *testing.T) {
	tests := []compilerTestCase{
		...
		},
		{
			...
			expectedConstants: []interface{}{
				55,
				[]code.Instructions{code.Make(code.OpConstant, 0),
...
func TestCompilerScopes(t *testing.T) {
	...
	compiler.emit(code.OpSub)
	instructionLen := len(compiler.scopes[compiler.scopeIndex].instructions)
	if instructionLen != 1 {

		t.Errorf("instructions length wrong. got=%d", instructionLen)
	}
	last := compiler.scopes[compiler.scopeIndex].lastInstruction
	if last.Opcode != code.OpSub {
		t.Errorf("lastInstruction.Opcode wrong. got=%d, want=%d", last.Opcode, code.OpSub)
	...
	compiler.emit(code.OpAdd)
	twoLength := len(compiler.scopes[compiler.scopeIndex].instructions)
	if twoLength != 2 {
		t.Errorf("instructions length wrong. go=%d", twoLength)
	}
	last = compiler.scopes[compiler.scopeIndex].lastInstruction
	if last.Opcode != code.OpAdd {
		t.Errorf("lastInstruction.Opcode wrong. got=%d wanted=%d", last.Opcode, code.OpAdd)
	}
	previous := compiler.scopes[compiler.scopeIndex].previousInstruction
	if previous.Opcode != code.OpMul {
		t.Errorf("previousInstruction.Opcode wrong. got=%d, want=%d", previous.Opcode, code.OpMul)
	...
	tests := []compilerTestCase{
		...
		},
		{
			...
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpSetGlobal, 0),
				// Note what is the second arguments for
